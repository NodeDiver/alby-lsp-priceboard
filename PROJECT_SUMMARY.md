...
Last reviewed: 2025-09 • Repository snapshot analyzed locally
...
- Provide a stable, rate-limited public API with clear data-source signaling (live/cached/unavailable/mixed).
...
- Vercel KV persistence (per-channel-size keys + metadata + rolling history):
  - `alby:lsp:channel:{size}` for latest snapshot per size.
  - `alby:lsp:metadata` for summary info.
  - `alby:lsp:history` for recent history.
...
- Frontend components:
  - `PriceTable` with per-LSP status badges, retry/force controls, currency conversion using Alby Lightning Tools.
  - Channel size and currency selectors; non-blocking UI while refreshing.
- Deployment support:
  - Vercel config with daily cron for price fetch and function timeout.
...
- Caching strategy:
  - KV snapshots per channel size with TTL (1h) and history (last 50 entries).
...
- Debug API import mismatch:
  - `pages/api/debug.ts` imports `getDatabaseStatus` from `lib/db` which does not exist.
...
- Env var naming consistency:
  - Code checks both Vercel KV (`KV_REST_API_*`) and Upstash (`UPSTASH_REDIS_*`) names in different files; consolidate to one check via `Redis.fromEnv()` and a single canonical predicate.
...
- Cron cadence mismatch with docs:
  - `vercel.json` uses a daily cron; docs/UI imply 10-minute intervals in some places.
- Type-safety bypass in production:
  - `next.config.ts` sets `typescript.ignoreBuildErrors: true`.
- Status inconsistency across docs:
  - `AGENTS.md` claims “Production Ready”; `README.md` still labels “Experimental phase”.
- API “estimated” data source mentioned in UI copy but not implemented in the backend response logic.
- No tests present despite “fully tested” claim.
...
1. Stabilization and Consistency
   - Implement `getDatabaseStatus` in `lib/db.ts` and fix `/api/debug` import.
   ...
   - Align docs and UI copy: remove “experimental” or add a flag that reflects reality.
   - Set `typescript.ignoreBuildErrors` back to `false` and fix type errors.

2. Scheduling and Operations
   - Align cron schedule with desired freshness (e.g., every 10–30 minutes) and add jitter per size to distribute load.
...
- Missing database status function (used by `/api/debug`):
  - pages/api/debug.ts:1
